...
from tf.bilstm_model import BiLSTM
from tf.bilstm_model import BiLSTMConfig
import os
import datetime
import time
...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        train_init_op, valid_init_op, next_train_element, next_valid_element = bilstm.prepare_data()
        bilstm.setBiLSTM()
        ...
        def valid_step(next_valid_element):
            ...
            while True:
                try:
                    ...
                    loss, pred, true = sess.run([bilstm.loss, bilstm.prediction, bilstm.labels], feed_dict)
                    y_pred.extend(pred)
                    y_true.extend(true)
                    valid_loss += loss
                    i += 1
